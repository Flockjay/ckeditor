              ...
              '</div>'
				...
				},
				{
					...
					url: [
						...
						/^(https:\/\/fj-user-uploads\.s3\.us-east-2\.amazonaws\.com\/fjvideo-([\w-]+)).([\w-]+)/
					],
					html: match => {
						const url = match[ 0 ];
						const ext = match[ 3 ];
				...
				},
				{
					name: 'Gong',
					url: [
						/^app\.gong\.io\/call\?id=([\w-]+)/,
						/^app\.gong\.io\/embedded-call\?call-id=([\w-]+)/
					],
					html: match => {
						const id = match[ 1 ];
						...
						return (
							'<div style="position: relative; padding-bottom: 100%; height: 0; padding-bottom: 56.2493%;">' +
								`<iframe src="https://app.gong.io/embedded-call?call-id=${ id }" ` +
									'style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;" ' +
									'frameborder="0">' +
								'</iframe>' +
							'</div>'
